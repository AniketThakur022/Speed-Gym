...
def main():
    ...
    manifest = {
        ...
        "resolution_modes": {
            "book_chapter_rule": "matched a taxonomy_v1.1 chapter_rule keyed on "
                                 "(book, chapter) — the preferred path, since one "
                                 "chapter title can mean different things in "
                                 "different books",
            "label_match": "no (book, chapter) rule existed, so the normalized "
                           "chapter label matched a taxonomy entry directly. This "
                           "is a FALLBACK and is weaker: a bare label match is not "
                           "book-scoped. A high count here is a prompt to add "
                           "explicit chapter_rules, not a second endorsed path.",
            "note": "These two counts are a measurement of HOW rows resolved, not a "
                    "claim that either taxonomy version is short. They sum to "
                    "taxonomy_resolved.",
        },
        "contracts": {
